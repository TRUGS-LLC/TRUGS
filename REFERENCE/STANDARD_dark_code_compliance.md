...
**Enforced by:** `trug compliance` CLI + CI gates (this repo, `tools/compliance_check.py`)
...
**Does not cover:** third-party vendored code (call it out with a waiver — see §8), auto-generated files (`ARCHITECTURE.md`, `AAA.md`, `CLAUDE.md` — produced by `trug-a-folder render`), build artifacts.
...
The check rules C1–C7 are defined below in §3–§6 and implemented in `trug compliance`:
...
- Pass `trug-a-folder check` (existing CORE validation — 16 rules)
...
These rules already exist via `trug-a-folder check`; the compliance standard incorporates them rather than re-specifying.

## 6. CI gate

### 6.1 Every PR runs `trug compliance`
...
- A `.trug.json` file that fails `trug-a-folder check`
...
> **Implementation status:** [DEFERRED] — The waiver mechanism is specified here for completeness. `trug compliance` v1.x does not yet read `.github/compliance-waivers.json` or support `--no-waivers`. Track implementation in a future issue.
...
- `trug compliance --no-waivers` flag produces the unwaived percentage. The baseline can track either the waived or the raw percentage, configured in `.github/compliance-baseline.json`.
