...
### Step G — Run `trug compliance` and iterate
...
`trug compliance` runs across the whole repo. It walks folder-by-folder, checks each source file against its folder's TRUG, checks that cross-graph edges point at nodes that actually exist in the referenced graphs, and reports a single overall percentage.
...
- Auto-generated files (`ARCHITECTURE.md`, `AAA.md`, `CLAUDE.md`) — produced by `trug-a-folder render`, compliance is the responsibility of the source TRUG
...
- **Phase 7 (TESTING)** — Step 4 of §2. Tests with TRUG/L comments. `trug compliance` passes locally.
...
During development, decisions that shape the code are captured as memories — for
example: *"We chose Fisher-Yates because uniform randomness matters for a casino shoe;
other in-place shuffles have subtle bias"*, recorded with a rule (*"use Fisher-Yates for
any card shuffle"*) and a rationale (*"uniform random is a correctness requirement, not
an optimization"*).

Capture and recall are driven by the private agent-OS substrate, not by any public
`trug` / `trug-a-folder` verb — the memory store is a TRUG graph the substrate maintains
...
1. **PR gate** — `trug compliance --strict` must pass. Compliance % may not decrease.
...
This section specifies the Python dialect of TRUGS compliance — the concrete rules that `trug compliance` enforces. It is the Python appendix of `STANDARD_dark_code_compliance.md`; readers seeking the normative text should refer to the Standard directly.
...
Refer to `STANDARD_dark_code_compliance.md` §3 and §6 for the C1–C7 check specifications. The `--help` output of `trug compliance` is the runtime reference.
