...
Every TRUG must satisfy 7 structural boundaries. An additional 7 compositional rules are enforced when a graph declares the \texttt{core\_v1.0.0} vocabulary capability.
...
TRUGS enforces 16 validation rules. Rules 1--9 are structural and always enforced. Rules 10--16 are compositional and enforced when the graph declares the \texttt{core\_v1.0.0} vocabulary capability.
...
\begin{itemize}
    ...
    \item Version control is built in: language v1.0.0 is a specific graph.
...
\begin{itemize}
    ...
    \item Opt-in behavior (compositional rules only fire when \texttt{core\_v1.0.0} declared)
...
\begin{itemize}
    \item \texttt{init} --- create an empty memory TRUG (declares \texttt{core\_v1.0.0})
