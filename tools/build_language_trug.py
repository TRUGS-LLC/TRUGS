...
# <trl>
# MODULE build_language_trug SHALL READ FILE spec_vocabulary THEN PRODUCE FILE language_trug.
# MODULE build_language_trug CONTAINS FUNCTION parse_spec AND FUNCTION build_trug AND FUNCTION main.
# FILE language_trug SHALL EQUAL A RECORD deterministic 'of FILE spec_vocabulary.
# </trl>
...
# <trl>
# PARTY caller SHALL SEND STRING DATA spec_text TO FUNCTION parse_spec.
# FUNCTION parse_spec SHALL READ DATA spec_text THEN RETURN ALL RECORD word.
# EACH RECORD word SHALL HAVE A RECORD number AND A RECORD part_of_speech AND A RECORD definition.
# </trl>
def parse_spec(spec_text: str) -> list[dict]:
    ...
    for line in spec_text.splitlines():
        ...
        if row and current_part:
            ...
            # <trl> FUNCTION parse_spec SHALL APPEND A RECORD word TO DATA records. </trl>
            records.append(
                {
                    "number": number,
                    "word": word,
...
# <trl>
# PARTY caller SHALL SEND ALL RECORD word TO FUNCTION build_trug.
# FUNCTION build_trug SHALL MAP ALL RECORD word TO RECORD graph THEN RETURN RECORD graph.
# RECORD graph SHALL CONTAIN A RECORD node 'for EACH RECORD word.
# </trl>
def build_trug(records: list[dict]) -> dict:
    ...
    for part in parts_order:
        for sub in subcats_order[part]:
            ...
            sub_records = [
                r
                for r in records
                if r["part_of_speech"] == part and r["subcategory"] == sub
            ]
    ...
    # Word nodes
    # <trl> FUNCTION build_trug SHALL MAP EACH RECORD word TO A RECORD node 'at BASE_WORD. </trl>
    for r in records:
        props = {
            "word": r["word"],
            "number": r["number"],
    ...
    trug = {
        ...
        "meta": {
            ...
            "word_count": len(records),
            "part_counts": {
                p: sum(1 for r in records if r["part_of_speech"] == p)
...
# <trl>
# AGENT cli SHALL READ DATA argv THEN RETURN INTEGER DATA exit_code.
# WHEN DATA argv SHALL_NOT EQUAL 2 RECORD argument THEN FUNCTION main SHALL RETURN 2.
# FUNCTION main SHALL READ FILE spec THEN WRITE FILE out_json.
# </trl>
def main(argv: Optional[list] = None) -> int:
    ...
    # <trl> FUNCTION main SHALL WRITE DATA trug TO FILE out_path AS RECORD json. </trl>
